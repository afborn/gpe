...
def add_seq_results_to_df(df):
    '''Requires df to contain columns "EDITED CLONES - function goes through text files
    and extracts sequence for to add to df"'''
    df["SEQUENCE"] = ""
    for entry in range(len(df)):
        fname = df["EDITED CLONES"].iloc[entry]
        f = open(fname, 'r')
...
def translate_df_entry_to_protein(df):
    ...
    for row in range(len(df)):
        ...
        for i in range(3):
#            protein_intermediate = []
            frames = dna_seqs[i:]
            aa_seq.append(translate(frames))
            #protein_intermediate.append(flatten_list(aa_seq))
            edited_clones.append(df["EDITED CLONES"].iloc[row])
...
#         for i in range(3):
#             edited_clones.append(df["EDITED CLONES"].iloc[row])
    df_aa = pd.DataFrame(zip(edited_clones, protein_seq_final), columns=('EDITED CLONES', 'AA_SEQ'))
    return df_aa
